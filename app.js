...
function renderCard(p, idx) {
  ...
  const mapBtn  = addr ? `<button class="action-circle action-circle--map js-map" data-addr="${esc(addr)}" title="Directions">
  ...
  <div class="card-head">
    ...
    <div class="card-info">
      ...
      <div class="card-meta">
        ...
        ${addr ? `<div class="card-addr"><span>📍</span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(addr)}</span></div>` : ''}
...
function extractWithRegex(textData, filename) {
  ...
  // ══ 3. DOSE ══
  // Strategy A: bold PDF items (most reliable — selected dose is bold/underlined in form)
  const doseMap = [
    { val:'45',   re:[/45\s*mg/i,   /6[\s\-]m/i] },
    { val:'30',   re:[/30\s*mg/i,   /4[\s\-]m/i] },
    { val:'22.5', re:[/22\.5\s*mg/i,/3[\s\-]m/i] },
    { val:'7.5',  re:[/7\.5\s*mg/i, /1[\s\-]m/i] },
  ];
  const boldLine = boldItems.join(' ');
  for (const { val, re } of doseMap) {
    if (re.some(r => r.test(boldLine))) { result.dose = val; break; }
  }
  // Strategy B: dose that appears more than once (selected option often repeated on form)
  if (!result.dose) {
    for (const { val, re } of doseMap) {
      const count = re.reduce((n,r) => n + (text.match(new RegExp(r.source,'gi'))?.length||0), 0);
      if (count > 1) { result.dose = val; break; }
    }
  }
  // Strategy C: any match in text (last resort, least specific)
  if (!result.dose) {
    for (const { val, re } of doseMap) {
      if (re.some(r => r.test(text))) { result.dose = val; break; }
    }
  }
  ...
  // Phone 2: SECONDARY CONTACT row — extract the last phone on that line (usually patient cell)
  // e.g.  "(Wife) Claire Forget // patient's cell: 514 916-7340"
  const secBlock = text.match(/SECONDARY\s*CONTACT[^#\n]*(?:#[^\n]*)?\n([^\n\r]+)/i) ||
                   text.match(/CONTACT\s*SECONDAIRE[^\n]*\n([^\n\r]+)/i);
  if (secBlock) {
    const phones = [...secBlock[1].matchAll(/(\d{3}[\s\-\.]?\d{3}[\s\-\.]?\d{4})/g)];
    if (phones.length) result.phone_2 = phones[phones.length-1][1].trim();
  }
  // Also try inline on same line as label (some forms put it there)
  if (!result.phone_2) {
    const secInline = text.match(/SECONDAIRE[^\n]*(\d{3}[\s\-\.]?\d{3}[\s\-\.]?\d{4})/i);
    if (secInline) result.phone_2 = secInline[1].trim();
  }
